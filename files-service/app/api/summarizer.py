...
# generate_presigned_url being omitted for now, but this is the function that should be used to generate links
# note that for generate_download_url to work, the download permissions are set to lax in the minio server and we do not want this
from .file_handling import list_files, generate_presigned_url, generate_download_url
...
# doing something similar in file handling - makes sense that this happens there
@router.get("/get_summaries/", tags=["submissions"])
# replace with call to minio endpoint
async def get_summaries():
    # uploader here is the bucket name - each bucket name will have to be retrieved from a db record for each client
    # similarly submissions is a folder name that will have to be defined per client
    files_info = await list_files('reports')
    files = files_info["files"]
    
    file_objects = []
    for filename in files:
        file_path = await generate_presigned_url(filename)        
        # file_path = await generate_download_url(filename)
        file_objects.append({
            "filename": filename, 
            # this would be the time when the file was uploaded - probably handled by minio?
